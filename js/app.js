...
function addSk() {
  ...
  if (!v || portData.skills.includes(v)) {
    inp.value = "";
    return;
  }
...
function renderStep() {
  ...
  document.getElementById("step-label").textContent =
    `Step ${currentStep + 1} of 5 — ${stepNames[currentStep]}`;
  for (let i = 0; i < 5; i++) {
    const d = document.getElementById(`sd-${i}`);
    d.className =
      "sdot" + (i < currentStep ? " done" : i === currentStep ? " active" : "");
  }
  document.getElementById("fbk").style.display =
    currentStep > 0 ? "block" : "none";
  document.getElementById("fnxt").textContent =
    currentStep === 4 ? "📋 Copy & Finish" : "Continue →";
}
function nextStep() {
  if (currentStep === 4) {
    copyHTML();
